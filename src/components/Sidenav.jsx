...
const Sidenav = ({ changeDarkMode, darkMode }) => {
  ...
  return (
    <div>
      {nav ? (
               <FaTimes className="fixed m-4 right-4 z-[99] md:hidden cursor-pointer bg-gray-400 dark:bg-gray-700 p-1 w-7 h-auto rounded-lg dark:text-white hover:scale-110 transition duration-100" size={20} onClick={handleNav} />
              ) : (
                <AiOutlineMenu
        size={20}
        onClick={handleNav}
